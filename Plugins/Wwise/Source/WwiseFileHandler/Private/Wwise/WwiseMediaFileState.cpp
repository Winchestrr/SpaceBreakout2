...
void FWwiseStreamingMediaFileState::OpenFile(FOpenFileCallback&& InCallback)
{
	...
	{
		UE_LOG(LogWwiseFileHandler, Error, TEXT("Media %" PRIu32 " (%s): WwiseFileCache not available."), MediaId, *DebugName.ToString());
		OpenFileFailed(MoveTemp(InCallback));
		return;
	}
	...
	FileCache->CreateFileCacheHandle(StreamedFile, FullPathName, PrefetchSize, StreamingGranularity, [this, Callback = MoveTemp(InCallback)](bool bResult) mutable
	{
		if (UNLIKELY(!bResult))
		{
			UE_LOG(LogWwiseFileHandler, Error, TEXT("Media %" PRIu32 ": Failed to load Streaming Media (%s)."), MediaId, *DebugName.ToString());
			delete StreamedFile; StreamedFile = nullptr;
			OpenFileFailed(MoveTemp(Callback));
			return;
		}

		iFileSize = StreamedFile->GetFileSize();
		OpenFileSucceeded(MoveTemp(Callback));
	});
}
...
AKRESULT FWwiseStreamingMediaFileState::ProcessRead(
