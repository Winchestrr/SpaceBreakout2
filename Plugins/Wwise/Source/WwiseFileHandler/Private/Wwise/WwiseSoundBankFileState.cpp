...
void FWwiseInMemorySoundBankFileState::FreeMemoryIfNeeded()
{
	...
	if (!LoadAsMemoryView())
	{
		if (MappedHandle)
		{
			UE_LOG(LogWwiseFileHandler, VeryVerbose, TEXT("SoundBank %" PRIu32 " (%s)::FreeMemoryIfNeeded: Freeing Mapped Handle"), SoundBankId, *DebugName.ToString());
			if (LIKELY(MappedRegion))
			{
				UnmapRegion(*MappedRegion);
			}
		...
		{
			UE_LOG(LogWwiseFileHandler, VeryVerbose, TEXT("SoundBank %" PRIu32 " (%s)::FreeMemoryIfNeeded: Freeing Pointer"), SoundBankId, *DebugName.ToString());
			DeallocateMemory(Ptr, FileSize, bDeviceMemory, MemoryAlignment, bContainsMedia);
		}
		Ptr = nullptr;
		FileSize = 0;
