...
void FWwiseStreamedExternalSourceFileState::OpenFile(FOpenFileCallback&& InCallback)
{
	...
	{
		UE_LOG(LogWwiseFileHandler, Error, TEXT("External Source %" PRIu32 " (%s): WwiseFileCache not available."), MediaId, *MediaPathName.ToString());
		OpenFileFailed(MoveTemp(InCallback));
		return;
	}
	...
	FileCache->CreateFileCacheHandle(StreamedFile, FullPathName, PrefetchSize, StreamingGranularity, [this, Callback = MoveTemp(InCallback)](bool bResult) mutable
	{
		if (UNLIKELY(!bResult))
		{
			UE_LOG(LogWwiseFileHandler, Error, TEXT("External Source %" PRIu32 ": Failed to load Streaming Media (%s)."), MediaId, *MediaPathName.ToString());
